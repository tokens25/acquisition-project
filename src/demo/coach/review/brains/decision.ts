    ...
    for (const p of s.plans) {
      ...
      out.push(
        finding({
          ...
          interpretation: `To know what this plan gives you, you have to go back and read ${named.map((o) => o.name).join(' and ')}. Saying it this way also sounds like a lot, which may be why it is written like that. Naming the teams instead is clearer, but then people can count them and see where it stops.`,
          recommendation: null,
          expectedMechanism: 'Sounding big may attract; being clear may convince. Which one wins here is unknown.',
          confidence: 'low',
          severity: 'test',
          test: proposeTest({
            hypothesis: `Whether ${p.name} is described as “everything in the other plans” or by naming its teams changes how often people pick it.`,
            science: 'Framing Effect, and Loss Aversion on the limit a list makes visible',
            observation: `Today it names the other plans, and the badges under it show the ${logos} teams anyway.`,
            ...
            learn: `Which way of describing ${p.name} people respond to. Science alone cannot say.`,
            goalRelevant: false,
