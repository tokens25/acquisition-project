...
export function goalBrain(s: JourneySnapshot, ctx: CoachReviewContext): Finding[] {
    ...
    if (present.length === 0) {
      ...
      out.push(
        finding({
          ...
          expectedMechanism: 'The reason someone came stays in front of them all the way to paying.',
          confidence: 'medium',
          severity: 'check',
          validation: 'none',
          goals: { [g]: -1 },
    ...
    if (!yearly) {
      ...
      out.push(
        finding({
          ...
          expectedMechanism: 'Whatever is already ticked gets chosen far more often.',
    ...
    if (!t) askFor('drive-benefit', 'which benefit')
      ...
      out.push(
        finding({
          ...
          expectedMechanism: 'Saying what you get, where the choice is made, gives people a reason to choose it.',
          confidence: 'high',
  ...
  out.push(
    finding({
      ...
      evidence: [],
      interpretation: `I judge how prices are shown and compared, never whether they are the right prices. I will not raise, lower or invent one.`,
      recommendation: null,
      confidence: 'high',
      severity: 'note',
      goals: {},
