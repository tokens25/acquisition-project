...
| **browser chrome** | `.jf__chrome` | The address bar at the foot of a tile, where iOS puts it and where every Figma frame draws it. It is pinned rather than left in the picture, so it holds still while the screen scrolls behind it — the exported bar is clipped off and this one drawn in its place. |
